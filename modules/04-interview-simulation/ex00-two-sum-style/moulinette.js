...
test('REASONING.md exists and has content', () => {
  ...
  assert(lower.includes('brute') || lower.includes('brute-force'), 'Should mention brute-force approach');
...
try {
  ...
  console.error(`\x1b[31mKO: Failed to require two_sum.js: ${error.message}\x1b[0m`);
  process.exit(1);
}

test('Exports twoSum', () => {
  assert(moduleExports, 'Module exports should exist');
  assert(typeof moduleExports.twoSum === 'function', 'twoSum should be exported');
});
...
test('twoSum handles array with duplicates', () => {
  ...
  assert.notStrictEqual(result[0], result[1], 'Should not use same element twice');
});
